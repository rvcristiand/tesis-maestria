...
Este programa es una extensión de \emph{TABS} (Three Dimensional Analysis of  Building Systems) para permitir analizar pórticos en tres dimensiones. Según \cite{ETABS1972}, una de las razones para desarrollar este programa fue darle una retroalimentación a los usuarios de cuatro programas diferentes que se usaban en varias estructuras importantes en diferentes países.\\

Estos programas, \emph{FRMSTC}, \emph{FRMDYN}, \emph{LATERAL} y \emph{SAP}, desarrollados en la Universidad de California, Berkeley, permitian el análisis lineal de edificios de varios pisos. FRMSTC (\emph{Static Load Analysis of High-Rise Buildings}) permitia analizar edificios simetricos con pórticos y muros a cortante paralelos sujetos a cargas estáticas. Las formas de los modos y las frecuencias también eran evaluadas. FRMDYN (\emph{Dynamic Analysis of Multistory Buildings}) era el mismo que FRMSTC con la excepción que la carga es la aceleración del terreno debido a un desplazamiento dependiente del tiempo. LATERAL fue una extensión de FRMSTC que permitia analizar linealmente pórticos y muros a cortante que no eran necesariamente paralelos. Habían tres grados de libertad en cada piso. Este programa no tenía la opción de realizar análisis dinámico. SAP (\emph{Static Analysis Program}) era un programa general para el análisis de vigas complejas y de elementos finitos y tenía una opción que permitía introducir la aproximación de piso rígido. Este programa también tenía la opción de realizar el análisis dinámico. \\
